...
from loading import load_sliced_depth_maps
...
def run_stress_testing(data_dir, save_name="domain_randomised", visual_save=True):
    output_dir = os.path.join(data_dir, f"depth_map_{save_name}")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    transformations = get_transformations()

    for i, img in enumerate(apply_sequential_transforms(load_sliced_depth_maps(data_dir), transformations)):
        # Masks are binary...
        img_to_save = cv2.cvtColor(np.array(np.clip(img, 0, 255), dtype=np.uint8), cv2.COLOR_RGB2BGR)
        if not visual_save:
